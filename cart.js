const items = document.querySelector(".cartDiv");
  ...
  try {
    ...
    data.products.forEach(async (item, index) => {
      ...
      <a href="/productItem.html?id=${item.id}&category=${item.category}">
          <img class="p-5 w-[320px] h-[250px] rounded-t-lg" src="${item.thumbnail}" alt="${item.title}" data-index="${item.id}" />
      </a>
      <div class="px-5 pb-5">
          <a href="#">
          <h3 class="text-xl font-semibold tracking-tight text-gray-900 dark:text-white h-[30px]">${item.title}</h3>
          </a>
          ...
          <div class="flex items-center justify-between">
              <span class="text-3xl font-bold text-gray-900 dark:text-white text-[23px]">$ ${discountPrice.toFixed(2)}<sup class="text-[14px]">$<del>${item.price}</del></sup></span>
...
export async function discountPriceFun(price, discountPercent) {
